...
class KVPoolScheduler:
    ...
    def get_num_new_matched_tokens(
        ...
        if (
            ...
            and not self.consumer_is_to_load
        ):
            return 0, False
    ...
    def update_state_after_alloc(
        ...
        load_specs = self.load_specs[request.request_id]
        kvpool_cached_tokens = load_specs.kvpool_cached_tokens
        vllm_cached_tokens = load_specs.vllm_cached_tokens
        external_matched_tokens = kvpool_cached_tokens - vllm_cached_tokens
        assert (
            num_external_tokens > 0
            and num_external_tokens == external_matched_tokens
        ), (
            f"Mismatch in number of tokens: {num_external_tokens} vs "
            f"{kvpool_cached_tokens} - "
            f"{vllm_cached_tokens}"
            ...
            for i, req_id in enumerate(cached_reqs.req_ids):
                ...
                if req_tuple:
                    ...
                    new_token_ids = request.all_token_ids[
                        num_current_tokens:
                        num_current_tokens + num_new_tokens
                    ]
